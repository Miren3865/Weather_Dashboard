...
api_key = '2c2f9a3908d0ed90a95f4f49e5a9169f';
...
function getWeatherDetails(name, lat, lon, country, state) {
    ...
    fetch(AIR_POLUTION_API_URL).then(res => res.json()).then(data => {
        ...
        alert(`failed to fetch air details for ${name}`);
    });
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data => {
        ...
        alert(`failed to fetch current details for ${name}`);
    });
    ...
    fetch(FORECAST_API_URL).then(res => res.json()).then(data => {
        ...
        alert(`failed to fetch weather forecast details for ${name}`);
    });
}
